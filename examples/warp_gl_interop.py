...
pos = p.TinyVector3f(0.0, 0.0, 1.0)
...
def init_vec(a: wp.array(dtype=wp.vec4)):
    tid = wp.tid()
    a[tid] = wp.vec4(0.0, 0.0, float(tid) * 1.5, 1.0)


vec_pos = [p.TinyVector3f(0.0, 0.0, 0.0)] * num_objects
vec_orn = [p.TinyQuaternionf(0.0, 0.0, 0.0, 1.0)] * num_objects
vec_scaling = [p.TinyVector3f(1.0, 1.0, 1.0)] * num_objects
vec_color = [p.TinyVector3f(1.0, 1.0, 1.0)] * num_objects
vec_scaling = [p.TinyVector3f(0.1, 0.1, 0.1)] * num_objects

sphere = app.renderer.register_graphics_instances(
    shape, vec_pos, vec_orn, vec_color, vec_scaling, opacity, rebuild
)
...
ground_plane = True
if ground_plane:
    pos = p.TinyVector3f(0.0, 0.0, 0.0)
    scaling = p.TinyVector3f(1.0, 1.0, 1.0)
    app.renderer.register_graphics_instance(
        shape, pos, orn, color, scaling, opacity, rebuild
    )
...
def init_kernel(a: wp.array(dtype=wp.vec4), sim_spacing: float, square_id: int):
    tid = wp.tid()
    a[tid] = wp.vec4(
        sim_spacing * float(tid % square_id) - float(square_id) * sim_spacing / 2.0,
        sim_spacing * float(tid / square_id) - float(square_id) * sim_spacing / 2.0,
        0.0,
        1.0,
    )
...
wp.launch(
    ...
    device=device,
    dim=num_objects,
    inputs=[positions, sim_spacing, int(math.sqrt(num_objects))],
)
app.cuda_unmap_vbo()
...
if 1:
    ...
    vertices_src = wp.array(
        ptr=vbo.vertices,
        dtype=wp.vec4,
        shape=(total_vertices,),
        strides=(vertex_stride,),
        length=total_vertices,
        capacity=total_vertices,
        device=device,
        owner=False,
        ndim=1,
    )
    ...
    x_y_dim = sim_spacing * math.sqrt(num_objects) / 2.0
    wp.launch(
        modify_vertices_kernel,
        device=device,
        dim=num_vertices,
        inputs=[vertices_src, vertices_dest, offset, x_y_dim],
    )
...
@wp.kernel
def test_kernel(a: wp.array(dtype=wp.vec4), b: wp.array(dtype=wp.vec4), dt: float):
...
while not app.window.requested_exit():
    ...
    with nvtx.annotate("sync_visual_transforms", color="orange"):
        ...
        wp.launch(
            ...
            device=device,
            dim=num_objects,
            inputs=[positions, velocities, 1.0 / 240.0],
        )
        app.cuda_unmap_vbo()
