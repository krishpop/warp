...
class CubeSlopeSim:
    ...
    def forward(self, inputs, compute_grad=True):
        ...
        for e in range(self.num_envs):
            ...
            builder.add_shape_box(body=body_id, hx=hx, hy=hy,
                                  hz=hz, density=density, ke=2.e+3, kd=0.0, mu=0.2)
...
#        wp.capture_begin()
        with wp.ScopedTimer("simulate", detailed=False, print=False, active=True, dict=profiler):
            ...
            if compute_grad:
                tape.backward(grads={self.loss: wp.array(
                    [1, 0, 0]*self.num_envs, dtype=wp.float32, device=self.device)})
                grad_mass = wp.to_torch(
                    tape.gradients[self.model.body_inv_mass]).clone()
                tape.zero()
            else:
                grad_mass = None
        ...
        return wp.to_torch(self.loss)[0], grad_mass
