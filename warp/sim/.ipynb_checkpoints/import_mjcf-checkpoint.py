...
def parse_mjcf(
    ...
    def parse_body(body, parent):
        ...
        if len(joints) == 1:
            ...
            print(
                f"{joint_name} coord: {start_coord} dof: {start_dof} body index: {link}"
                f" body name: {body_name} body pos: {body_pos}"
            )
        ...
        for geom in body.findall("geom"):
            ...
            if geom_type == "sphere":
                ...
                # add mesh vertices to builder
                if add_particles:
                    for v in mesh.vertices:
                        p = wp.quat_rotate(geom_rot, v * scale) + geom_pos
                        builder.add_particle(p, (0.0, 0.0, 0.0), 0.0)
                elif add_mesh:
                ...
                # Check if adding soft mesh is correct?
