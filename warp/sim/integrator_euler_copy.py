...
def eval_triangles_body_contacts(
    ...
    # body positions, orientations, velocities of colliding bodies
    x0 = wp.transform_get_translation(body_q[c_body])
    r0 = wp.transform_get_rotation(body_q[c_body])

    v0 = wp.spatial_bottom(body_q[c_body])
    w0 = wp.spatial_bottom(body_q[c_body])
...
def eval_body_contacts(
    ...
    # transform point to world space
    cp = (
        wp.transform_point(X_wb, c_point) - n * c_dist
    )  # add on 'thickness' of shape, e.g.: radius of sphere/capsule
...
def compute_forces(model, state, particle_f, body_f):
    ...
    # triangle/body contacts
    if model.enable_tri_body_collisions and model.tri_count:

        wp.launch(
            kernel=eval_triangles_body_contacts,
            dim=model.tri_count * model.body_count,
            inputs=[
                model.body_count * model.tri_count,
                state.particle_q,
                state.particle_qd,
                model.tri_indices,
                state.body_q,  # body body positions
                state.body_qd,
                model.contact_body0,
