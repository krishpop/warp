    ...
    class SimRenderer_t(renderer):
        ...
        def __init__(
            ...
            **render_kwargs,
            ...
            # create USD stage
            super().__init__(
                path, scaling=scaling, fps=fps, upaxis=upaxis, **render_kwargs
            )
        ...
        def populate(self, model: warp.sim.Model):
            ...
            if self.show_rigid_contact_points and model.rigid_contact_max:
                self.contact_points0 = wp.array(
                    np.zeros((model.rigid_contact_max, 3)),
                    dtype=wp.vec3,
                    device=model.device,
                )
                ...
                self.contact_points0_colors = [
                    (1.0, 0.5, 0.0)
                ] * model.rigid_contact_max
                self.contact_points1_colors = [
                    (0.0, 0.5, 1.0)
                ] * model.rigid_contact_max
            ...
            if self.model.shape_count:
                ...
                shape_geo_thickness = model.shape_contact_thickness.numpy()
                shape_geo_is_solid = np.array([True for _ in shape_geo_scale])
                ...
                for s in range(model.shape_count - 1):
                    ...
                    if geo_hash in self.geo_shape:
                        ...
                        if geo_type == warp.sim.GEO_PLANE:
                            ...
                            if not geo_is_solid:
                                faces, vertices = solidify_mesh(
                                    geo_src.indices, geo_src.vertices, geo_thickness
                                )
                            else:
                    ...
                    self.add_shape_instance(
                        name, shape, body, X_bs.p, X_bs.q, scale, color
                    )
        ...
        def render(self, state: warp.sim.State):
            ...
            if self.model.particle_count:
                ...
                # render particles
                self.render_points(
                    "particles", particle_q, radius=self.model.soft_contact_distance
                )

                # render tris
                if self.model.tri_count:
                    self.render_mesh(
                        "surface", particle_q, self.model.tri_indices.numpy().flatten()
                    )
                ...
                if self.model.spring_count:
                    self.render_line_list(
                        "springs",
                        particle_q,
                        self.model.spring_indices.numpy().flatten(),
                        [],
                        0.05,
                    )
