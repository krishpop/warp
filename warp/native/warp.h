...
extern "C"
{
	...
	WP_API uint64_t mesh_create_host(wp::vec3* points, wp::vec3* points_grad, wp::vec3* velocities, wp::vec3* velocities_grad, int* tris, int num_points, int num_tris);
	WP_API void mesh_destroy_host(uint64_t id);
    WP_API void mesh_refit_host(uint64_t id);

	WP_API uint64_t mesh_create_device(void* context, wp::vec3* points, wp::vec3* points_grad, wp::vec3* velocities, wp::vec3* velocities_grad, int* tris, int num_points, int num_tris);
