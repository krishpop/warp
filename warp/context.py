...
class Device:
    ...
    def __init__(self, runtime, alias, ordinal=-1, is_primary=False, context=None):
        ...
        if self.ordinal == -1:
            ...
            if warp.config.enable_mempools_by_default:
                # enable if supported
...
def capture_end(device: Devicelike = None, stream: Stream = None) -> Graph:
    ...
    if not result:
        raise RuntimeError("Error occurred during CUDA graph capture. "
                           "This could be due to an unintended allocation or CPU/GPU synchronization event. "
                           f"Error details: {runtime.get_error_string()}")
