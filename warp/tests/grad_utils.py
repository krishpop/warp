...
def plot_state_gradients(
    states: list, figure_name: str = "state_grads.html", blacklist_vars=set(["body_q_temp", "body_qd_temp"])
):
    from plotly.subplots import make_subplots
    import plotly.graph_objects as go
    ...
    for i, (key, series) in enumerate(absmax.items()):
        ...
        fig.add_trace(
            go.Scatter(x=np.arange(len(val_series)), y=val_series, name=key, legendgroup=key, line=dict(color=color)),
            row=1,
            col=1,
        )
        fig.add_trace(
