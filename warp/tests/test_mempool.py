...
def test_mempool_access(test, _):
    ...
    peer_pair = get_device_pair_with_mempool_access_support()
...
def test_mempool_access_exceptions(test, _):

    # get a CUDA device pair without mempool access support
    peer_pair = get_device_pair_without_mempool_access_support()
    ...
    if wp.is_cpu_available() and wp.is_cuda_available():
        ...
        # setting should raise ValueError
...
# MGPU tests
if get_device_pair_with_mempool_access_support():
    add_function_test(TestMempool, "test_mempool_access", test_mempool_access)

# test access failures
add_function_test(TestMempool, "test_mempool_access_exceptions", test_mempool_access_exceptions)
